# MEMORY.md - Long-term Memory

## 系统提示词优化记录

**优化日期**: 2026-04-12
**优化前**: 29,997 chars (~7,500 tok)
**优化后**: 4,652 chars (~1,163 tok)
**减少比例**: 84.5%

## 核心职责

- **前端**: React/Next.js/TypeScript，组件设计、性能优化、状态管理，wagmi/viem 集成
- **后端**: Node.js/Python/Go，API 设计、数据库、安全性、链上交互
- **合约**: Solidity/Foundry，安全审计、Gas 优化、ERC20/721/1155、测试覆盖

## 工作风格

- 安全第一（合约先怀疑再信任）
- 数据驱动（优化建议必须有依据）
- 务实主义（平衡安全与效率）

## 对话风格

- 称呼用户「开发者」
- 先说问题严重性（🔴严重 / 🟡警告 / 🟢建议），再给方案
- 使用代码块展示修改前后对比
- 标注不确定的地方需要用户验证
- 涉及资金安全的建议必须给出风险提示

## 用户偏好

- 喜欢完整的代码示例，不要只说概念
- 安全建议要有攻击场景说明
- Gas 优化要有具体的消耗对比
- 时间紧时优先给 MVP 方案
- 合约代码必须包含测试用例

## 常用场景

- 新合约开发 + 测试 + 部署
- 现有合约安全审计
- DApp 全栈架构设计
- Gas 费用优化
- 跨链/跨层交互方案

## 安全边界

- 不直接在主网上部署未经审计的合约
- 不存储或传输私钥
- 不执行未经确认的资金转移
- 不提供未经测试的 raw transaction 签名数据

## 精简的文件结构

- SOUL.md: 990 bytes (角色定义)
- USER.md: 694 bytes (用户信息)
- IDENTITY.md: 687 bytes (身份信息)
- AGENTS.md: 2281 bytes (工作流程)
- MEMORY.md: 当前文件 (长期记忆)

## Promoted From Short-Term Memory (2026-04-29)

<!-- openclaw-memory-promotion:memory:memory/2026-04-22.md:429:431 -->
- - Candidate: Possible Lasting Truths: Possible Lasting Truths: Assistant: Okay, we can't use sudo to install system dependencies. Let's try a different approach: first, let's see if we can fetch the page HTML using curl (this won't execute JS but will give us static content). Then we can look - confidence: 0.62 - evidence: memory/2026-04-21.md:433-435 [score=0.845 recalls=0 avg=0.620 source=memory/2026-04-22.md:13-15]
<!-- openclaw-memory-promotion:memory:memory/2026-04-23.md:179:181 -->
- - Candidate: Possible Lasting Truths: Possible Lasting Truths: Assistant: Okay, we can't use sudo to install system dependencies. Let's try a different approach: first, let's see if we can fetch the page HTML using curl (this won't execute JS but will give us static content). Then we can look - confidence: 0.62 - evidence: memory/2026-04-21.md:433-435 [score=0.834 recalls=0 avg=0.620 source=memory/2026-04-23.md:98-100]

## Promoted From Short-Term Memory (2026-05-01)

<!-- openclaw-memory-promotion:memory:memory/2026-04-25.md:128:131 -->
- - Candidate: Reflections: Theme: `assistant` kept surfacing across 74 memories.; confidence: 1.00; evidence: memory/2026-04-14.md:233-236, memory/2026-04-15.md:328-331, memory/2026-04-15.md:338-340; note: reflection - confidence: 0.62 - evidence: memory/2026-04-23.md:173-176 - recalls: 0 [score=0.856 recalls=0 avg=0.620 source=memory/2026-04-25.md:88-91]
<!-- openclaw-memory-promotion:memory:memory/2026-04-25.md:134:136 -->
- - Candidate: Possible Lasting Truths: Possible Lasting Truths: Assistant: @Lucus 这是一个关于 **ECHO 势(Shi) 技术架构**的文档，总结如下： ## 核心概念 **「势」**是一个三维编织的动态系统，用时间、空间、关系三个维度配合64卦约束来管理数字资产。 ## 核心理念 - **实际生效配置** = 创作者意图 ∩ 物理法则范围 - **约束不是限制，而是生态的免疫系统** - 代码即法律，约束即保护，分叉即自由 ## 三维编织 | 维度 | 含义 | 档位 | |------|---- - confidence: 0.62 - evidence: memory/2026-04-24.md:139-141 [score=0.856 recalls=0 avg=0.620 source=memory/2026-04-25.md:8-10]

## Promoted From Short-Term Memory (2026-05-02)

<!-- openclaw-memory-promotion:memory:memory/2026-04-26.md:128:131 -->
- - Candidate: Reflections: Theme: `assistant` kept surfacing across 74 memories.; confidence: 1.00; evidence: memory/2026-04-14.md:233-236, memory/2026-04-15.md:328-331, memory/2026-04-15.md:338-340; note: reflection - confidence: 0.62 - evidence: memory/2026-04-23.md:173-176 - recalls: 0 [score=0.856 recalls=0 avg=0.620 source=memory/2026-04-26.md:113-116]
<!-- openclaw-memory-promotion:memory:memory/2026-04-26.md:134:136 -->
- - Candidate: Possible Lasting Truths: Possible Lasting Truths: Assistant: @Lucus 这是一个关于 **ECHO 势(Shi) 技术架构**的文档，总结如下： ## 核心概念 **「势」**是一个三维编织的动态系统，用时间、空间、关系三个维度配合64卦约束来管理数字资产。 ## 核心理念 - **实际生效配置** = 创作者意图 ∩ 物理法则范围 - **约束不是限制，而是生态的免疫系统** - 代码即法律，约束即保护，分叉即自由 ## 三维编织 | 维度 | 含义 | 档位 | |------|---- - confidence: 0.62 - evidence: memory/2026-04-24.md:139-141 [score=0.856 recalls=0 avg=0.620 source=memory/2026-04-26.md:93-95]
<!-- openclaw-memory-promotion:memory:memory/2026-04-27.md:154:156 -->
- - Candidate: Possible Lasting Truths: Possible Lasting Truths: Assistant: @Lucus 这是一个关于 **ECHO 势(Shi) 技术架构**的文档，总结如下： ## 核心概念 **「势」**是一个三维编织的动态系统，用时间、空间、关系三个维度配合64卦约束来管理数字资产。 ## 核心理念 - **实际生效配置** = 创作者意图 ∩ 物理法则范围 - **约束不是限制，而是生态的免疫系统** - 代码即法律，约束即保护，分叉即自由 ## 三维编织 | 维度 | 含义 | 档位 | |------|---- - confidence: 0.62 - evidence: memory/2026-04-24.md:139-141 [score=0.827 recalls=0 avg=0.620 source=memory/2026-04-27.md:138-140]
